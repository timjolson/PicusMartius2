...
CalcPid gripperPID;	//object to calculate PID

/****					range	kp		ki		kd		respTime*/
PIDstruct PID_gripper={0, 		0.0,	0.0,	0.0, 	40, };
...
MotorAxis::MotorAxis(int _encoderPin, int _signalPin, bool _reverse, 
	bool _encoderDirection ,short _tolerance, struct JOINT* _JOINTobj, CalcPid* _PIDobj)
{
...
void MotorAxis::Calibrate(){
	switch (calibrating){
		...
		case 7://calibrate edge
			Read();//read joint using calibrated encoder
			if (calPos==mOUT){		//if position to calibrate from is motor out
...
void MotorAxis::CalibrateMove(){
	if (direction==mOUT)	//if intended direction is motor out
	{
