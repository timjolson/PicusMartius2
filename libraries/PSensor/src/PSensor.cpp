...
// Base library type I2C
{
	...
	_address = 0x76; //set interface used for communication
...
float PSensor::GetTemperature()
{
    return getTemperature(CELSIUS, ADC_4096);
}
float PSensor::GetPressure()
{
    return getPressure(ADC_4096);
}

float PSensor::getTemperature(temperature_units units, precision _precision)
...
float PSensor::getPressure(precision _precision)
// Return a pressure reading units Pa.
{
    ...
    UpdateAverage(pressure_reported);
    
    pressure_reported = UpdateAverage(pressure_reported);
    
	return pressure_reported;
}

void PSensor::getMeasurements(precision _precision)
...
uint32_t PSensor::getADCconversion(measurement _measurement, precision _precision)
...
void PSensor::sendCommand(uint8_t command)
{	
...
void PSensor::sensorWait(uint16_t time)
...
float PSensor::UpdateAverage(float input)
{
    avg = 0.0;
